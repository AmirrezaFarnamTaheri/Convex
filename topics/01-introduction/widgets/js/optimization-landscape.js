 ...
 *
 * Description: A unified widget for Lecture 01 combining 1D convexity theory (Jensen's Inequality)
 *              and 3D optimization landscapes (Global vs Local Minima).
 *
 * Features:
 * - Tab 1: "The Definition" (1D interactive Jensen's inequality)
 * - Tab 2: "The Consequence" (3D landscape viewer with physics simulation)
 * - Pyodide integration for surface calculation
 * - D3.js for 1D plot, Three.js for 3D scene
 *
 * Version: 1.0.0
 */
...
export async function initOptimizationLandscape(containerId) {
    ...
    container.innerHTML = `
        <div class="widget-container">
            <div class="widget-controls" style="display: flex; justify-content: space-between; align-items: center; border-bottom: none;">
                <div class="tab-container" style="display: flex; gap: 8px;">
                    <button class="widget-btn tab-btn active" data-tab="definition">
                        <i data-feather="activity"></i> The Definition (1D)
                    </button>
                    <button class="widget-btn tab-btn" data-tab="consequence">
                        <i data-feather="layers"></i> The Consequence (3D)
                    </button>
                </div>
                <div id="control-panel-1d" class="control-panel">
                    <select id="func-select-1d" class="widget-select" style="width: 180px;"></select>
                    <button id="clear-1d" class="widget-btn small">Clear</button>
                </div>
                <div id="control-panel-3d" class="control-panel" style="display: none;">
                    <select id="func-select-3d" class="widget-select" style="width: 180px;"></select>
                    <button id="drop-marble" class="widget-btn primary small">Drop Marble</button>
                    <button id="reset-marble" class="widget-btn small" disabled>Reset</button>
                </div>
            </div>

            <div id="view-1d" class="view-section">
                <div class="widget-canvas-container" id="plot-container-1d" style="height: 450px;"></div>
                <div id="result-display-1d" class="widget-output" style="min-height: 60px;">
            ...
            <div id="view-3d" class="view-section" style="display: none;">
                <div class="widget-canvas-container" id="plot-container-3d" style="height: 450px; position: relative;">
                ...
                <div class="widget-output" style="text-align: center;">
                    <span style="color: var(--text-secondary);">Blue = Low Value (Minima) &bull; Red = High Value (Maxima) &bull; Drag to Rotate</span>
                </div>
            </div>
        </div>
    `;
    ...
    style.innerHTML = `
        .tab-btn { opacity: 0.7; border-bottom: 2px solid transparent; border-radius: 0; }
        .tab-btn.active { opacity: 1; border-bottom-color: var(--primary-500); background: var(--surface-2); }
    ...
    // --- 1D VISUALIZER LOGIC (The Definition) ---
    const functions1D = {
    ...
    function init1D() {
        ...
        window.addEventListener('resize', () => {
             if (state.currentTab === 'definition') {
                 setupChart1D();
                 draw1D();
             }
        });
    ...
    function draw1D() {
        ...
        // Redraw points/chord if exist
        const g = svg1D.select(".interaction");
        g.selectAll("*").remove();

        points1D.forEach(p => {
            g.append("circle")
                .attr("cx", xScale1D(p.x)).attr("cy", yScale1D(p.y))
                .attr("r", 6).attr("fill", "var(--accent-500)").attr("stroke", "white").attr("stroke-width", 2);
        });
    ...
    function drawChord1D() {
        ...
        // Chord
        g.append("line")
            .attr("x1", xScale1D(p1.x)).attr("y1", yScale1D(p1.y))
        ...
        const area = d3.area()
            .x(d => xScale1D(d.x))
            .y0(d => yScale1D(d.yChord))
            .y1(d => yScale1D(d.yCurve));

        let color = isConvex ? "var(--success)" : (isConcave ? "var(--warning)" : "var(--error)");
        let message = isConvex
            ? `<strong style="color: var(--success)">✓ Jensen's Inequality Holds:</strong> Chord is above curve. Locally Convex.`
    ...
    // --- 3D VISUALIZER LOGIC (The Consequence) ---
    const pythonCode = `
import numpy as np

FUNCTIONS = {
    "Convex Bowl (Global Min)": {
...
def calculate_surface(name, n=60):
    ...
    zz = spec['func'](xx, yy)
    return {"z": zz.tolist(), "domain": d}

def get_grad(name, x, y):
    spec = FUNCTIONS[name]
    z = spec['func'](x, y)
    g = spec['grad'](x, y)
    return {"z": float(z), "grad": g.tolist()}

list(FUNCTIONS.keys())
`;

    let scene, camera, renderer, surfaceMesh, marbleMesh, controls;
    ...
    async function init3D() {
        ...
        if (!state.pyodideReady) {
            ...
            state.pyodideAPI = {
                calc: state.pyodide.globals.get('calculate_surface'),
                grad: state.pyodide.globals.get('get_grad')
            };
        ...
        scene.background = new THREE.Color(getComputedStyle(document.body).getPropertyValue('--surface-1').trim());
    ...
    async function updateSurface3D() {
        ...
        let minZ = Math.min(...zFlat);
        let maxZ = Math.max(...zFlat);
        let range = maxZ - minZ || 1;
    ...
    function animate3D() {
        if (!document.getElementById('plot-container-3d')) return; // Stop if removed
    ...
    async function startMarble() {
        ...
        // Physics State
        // Start at top-left of domain equivalent
        let x = -1.8, y = -1.8; // Approx domain coords
        let vx = 0, vy = 0;
        ...
        const loop = async () => {
            ...
            // Map domain to visual range [-3, 3]
            // Assume domain is roughly 2.0 or 3.0, normalize to fit plane size 6
            // Simplified mapping for visual demo:
            const scale = 3.0 / 2.0; // Approx
